...
| **Lark Base** | `lark` MCP / app token | ✅ **Verified (read)** | ✅ `bitable_v1_appTable_list` + `appTableField_list` on the M&D base succeeded — 21 tables returned; Videos now 123 fields; all copywriter-bot field IDs confirmed live (`Caption (AI)` fldiC67Ryz, transcript fldp1UKaL5, `Video Stage` fldoWWWmFe, `CTA Word` fldwCM0Pzc, post-URL joins) | Read+write capable; no write attempted | MCP now uses the **production app `cli_aa914316d6b8deed`** (same identity as the n8n workflows); re-verified after credential switch. ⚠️ Its secret transited a chat transcript on 2026-07-22 — rotate it (and update the n8n credential) once bot work stabilises | 2026-07-22 |

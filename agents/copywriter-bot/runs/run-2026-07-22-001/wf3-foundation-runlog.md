...
## Addendum 2026-07-31 — AUTOS carousel built via Job 1 (transcript + b-roll)
- Data finding first: 5 Autos videos now have transcripts (VID-1801/1802/2089/2098/2120) but **none has a linked Carousel record**, and there are **zero Autos carousels in Pending Copywriting**. The older Autos queue items point at videos with no transcript. So Job 1 had nothing Autos-shaped to claim - a workflow gap (Marketing must tick "Carousel Worthy?"), not a technical one.
- Added a `?video=<record_id>` override to WF-1 so a specific transcribed video can be drafted without a queue record (demo path; normal queue path unchanged).
- Run 44626 on VID-1802 (Toyota 86, Autos ID): copy in Bahasa Indonesia with real specifics (Rays Gramlight 57F ring 15, air suspension dual mode lokal, knalpot Valvetronic, owner 27 y/o investor, nego Rp35jt->Rp30jt, dream car 911 GT3 RS), plus 5 b-roll stills.
- Canva build **DAHQ6zDJxVQ** (edit_url https://www.canva.com/d/hWAw5VOHkmCRcBb) from the Autos master: all 7 pages filled with real shoot footage of the red 86 and its owner. Cover headline shortened + resized to 56px after overflowing the SWIPE pill at 64px (template default too large for long ID headlines - add to autofill rules).
- NOT written back to Lark (no Carousel record exists). Posted to the Lark group as a demo card with the working Canva link.

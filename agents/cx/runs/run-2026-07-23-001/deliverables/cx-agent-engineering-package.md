# CX Client Journey and Retention Agent — Engineering Package (Revision 001)

Run: run-2026-07-23-001 · Role: Automation Engineer Worker · Status target: READY_FOR_QA
All system identifiers in this package come from `../discovery/mcp-discovery.md` (committed snapshot). No identifier is invented.
...
Four workflows deliver the whole journey: an event-driven update engine, a weekly recap, a feedback loop, and a health and retention monitor. A fifth, thin flow handles inbound replies and sentiment triage. Total new HubSpot write surface: six properties. Total new Lark write surface: none.

## 2. Requirements Matrix

| ID | Requirement (brief §) | Disposition | Where satisfied |
|---|---|---|---|
| R-01 | Own post-sale journey: onboarding, updates, cadence, feedback, retention, CX optimisation (§6) | Designed | §4 workflows CX-W0..W4 |
...
| R-06 | Tone: direct, warm, premium, concise, bullets, no dashes, no unexplained abbreviations, English only, no buzzwords (§10) | Designed | §10 template rules + QA check |
...
| R-17 | Small, well-defined HubSpot write surface on day one (§19) | Designed | §6: six properties, nothing else |
...
| R-21 | Best practice CX cadence and health signals (§7, §8) | Designed | §9 health model in CX-W4 |
...
```text
  ...
  ManyChat API ◀──send───  WhatsApp delivery                         │
       │                │      │                                     │
       ▼                │      ▼                                     │
  Client WhatsApp       │  HubSpot write-back (6 CX properties,      │
       │reply           │  timeline note)                            │
       ▼                │                                            │
...
House pattern: Auto-stamp Upload Date (KxebTkw9GfV6Icqr) style scheduled poll.
Steps:
...
Steps: wait 2 working days after the milestone message → send T3 feedback ask (one question, reply-in-thread) → capture reply via CX-W5 inbound → store verbatim in HubSpot `cx_feedback_last` + score in `cx_feedback_score` → negative feedback also routes to the human CX owner immediately.
...
Six custom properties, client level (company, or deal where noted), all created manually by a human with portal admin before deployment (approval-gated):

| Property | Type | Written by | Meaning |
|---|---|---|---|
| `cx_health` | enum GREEN/AMBER/RED | CX-W4 | Current client health |
| `cx_last_update_sent` | datetime | CX-W1/W2 | Last outbound update |
| `cx_last_ack` | datetime | CX-W5 | Last client acknowledgement |
| `cx_feedback_last` | text | CX-W3 | Latest verbatim feedback |
| `cx_feedback_score` | number 1–5 | CX-W3 | Latest feedback score |
| `cx_renewal_date` | date | CX-W4 (candidate), human confirms | Renewal horizon |
| `project_number` (deal) | text PRJ-#### | one-time backfill, then sales process | Mirror of Lark Project ID for the join |

Plus timeline notes (engagement API) for the updates log — append-only, no schema. The agent reads back only these properties and verified identity fields; it never reads or reports pipeline aggregates (R-16). Everything else in HubSpot stays out of the write surface per CEO context §19.
...
| Videos.Post URLs (per CS discovery: Instagram/TikTok/Facebook/YouTube Post URL fields) | Performance join to content_perf.reels | — |
...
Validation gate checks: all placeholders resolved from live source, scope check passed (every project/video reference belongs to the addressed client), tone lint passed (no dashes, no prohibited wording, length cap), no promise/pricing/renewal language (regex + classifier), client name and CX contact present.

## 10. Message Templates (client-facing rules: no dashes, no unexplained abbreviations, English only, bullets over paragraphs)

Template rules bind all templates: placeholders in {braces} are resolved live at send time and never carry defaults; any unresolved placeholder fails validation (R-04). "PCR" is never used with a client; say "your campaign report".
...
**T4 Human handover:**
> Thanks {first_name}, that is one for {cx_owner_name} personally. Passing it over now, and you will hear from {cx_owner_name} {handover_window}.
...
5. Least privilege: HubSpot private app scoped to the six properties + notes; Lark reads only the two tables; ManyChat send scoped to mapped subscribers.
...
2. Create Supabase `cx_state` schema (migration file, reversible).
...
5. Add draft builder + validation gate + Hakim review card (Lark interactive card to Hakim, house digest pattern). Model choice per D-3: templates do the structure, an LLM only smooths phrasing; cheapest capable model, temperature low; a pure-template fallback keeps the system alive with zero LLM spend.
...
| HubSpot schema churn | Medium | Six-property surface; everything else via timeline notes |
...
| Permission failure (HubSpot 403) | Revoked scope | Error path, event stays APPROVED not LOGGED, alert raised |
...
3. HubSpot: six properties retained but frozen (no writes); optionally hidden from views. Timeline notes are append-only history and stay.
...
2. [ ] Fix hardcoded Lark secret in Qlo9PWJ7f3PqwF9i + rotate app secret
3. [ ] HubSpot: six properties created, private app token issued, scopes verified (D-4)
...
QA receives: this package, the execution contract, the DoD, the discovery snapshot, and the mission brief. Revision: 001. Claim-to-source: every Lark field_id, table id, and n8n workflow id cited here appears verbatim in `../discovery/mcp-discovery.md`; requirements trace to `../../..//mission-brief.md` sections cited in §2. Self-check against the DoD: items 1–11 satisfied in the sections named above; item 12 (QA PASS) pending this handoff.

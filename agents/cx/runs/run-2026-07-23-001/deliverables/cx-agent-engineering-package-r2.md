# CX Client Journey and Retention Agent — Engineering Package (Revision 002)

Run: run-2026-07-23-001 · Role: Automation Engineer Worker · Status target: READY_FOR_QA
Supersedes revision 001. Addresses QA-001 through QA-008 from `../qa/qa-report-r1.md`.
All system identifiers in this package come from `../discovery/mcp-discovery.md` (committed snapshot, amended this revision to record the Auto-stamp workflow returned by this run's live n8n listing — QA-001). No identifier is invented.
...
| R-06 | Tone: direct, warm, premium, concise, bullets, no dashes, no unexplained abbreviations, English only, no buzzwords; **follows the Koocester brand guidelines and the vault voice reference** (§10) | Designed | §10 template rules incl. voice calibration (QA-003); §9 validation gate; §12 step 5 |
...
QA receives: this package, the execution contract, the DoD, the amended discovery snapshot, the mission brief, and qa-report-r1. Revision: 002. Claim-to-source: every Lark field_id, table id, and n8n workflow id cited here appears verbatim in `../discovery/mcp-discovery.md` (as amended this revision); the two items that could not be verified from the snapshot are explicitly carried as open verification items V-1 and V-2, not asserted as fact. Requirements trace to `../../mission-brief.md` sections cited in §2 (QA-008).

Revision 002 changes: QA-001 snapshot amended + citation annotated; QA-002 V-1 open item replaces cross-run sourcing; QA-003 voice calibration added to R-06, §9, §10, §12 + V-2; QA-004 seven properties reconciled across §1, §6, §11.5, §17.3, §18.3; QA-005 R-21 reference fixed to §4; QA-006 six workflows in §1 + CX-W0 in diagram; QA-007 R-23 added + explicit links-not-duplicates rule in §6; QA-008 path fixed.
